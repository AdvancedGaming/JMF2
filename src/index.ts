...
bot.on("withdraw", async (interaction, coins, cards, packs) => {
    ...
    try {
        ...
        interaction.createFollowup("Withdraw was cancelled, Get better bozo");
        return;
...
bot.on("ga-announce", async (interaction, start, duration) => {
    ...
    giveawayMessage = await bot.sendMessage(channelId, {
        ...
        content: `<@&${bot.config.giveawayPingRoleId}>, a ${duration ? duration + " minute long " : ""}giveaway is starting <t:${startTime}:R>! Make sure to be linked or you will not get any trades. To enter this giveaway you need to react to the :tada:`
    });
...
async function giveawayStart() {
    ...
    if (giveawayMessage) {
        await bot.editMessage(giveawayMessage.channel.id, giveawayMessage.id, {
            content: `Signups for this giveaway are now closed! The giveaway will be starting shortly. Make sure to be linked, to learn how to link please go to #🤖-bot-introduction and watch the linking video!! `,
            components: []
        });
    }
    ...
    await bot.sendMessage(giveawayMessage.channel.id, {
        ...
        content: `<@&${bot.config.giveawayPingRoleId}>, the ${rawGiveawayDuration ? rawGiveawayDuration + " minute long " : ""}giveaway has started with **${giveawaySignups.length} people**! Look at your invites and trade as many times as you can! and hope you enjoy guys!!`
    });
...
async function giveawayEnd(channelId: string) {
    ...
    bot.sendMessage(channelId, {
        ...
        content: `<@&${bot.config.giveawayPingRoleId}>, the giveaway has ended! Hope you enjoyed it, We will do more!! Do not worry!`
    });
}
